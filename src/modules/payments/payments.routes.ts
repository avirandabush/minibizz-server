...
import * as validation from './payments.validation';
import { handleError } from '../../utils/handleError';
...
router.get('/', async (req, res) => {
    try {
        const data = await service.getPayments((req as any).userId);
        res.json(data);
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        const data = await service.getPaymentById((req as any).userId, req.params.id);
        if (!data) return res.status(404).json({ error: 'Payment not found' });
        res.json(data);
    } catch (err) {
        handleError(res, err, 'Failed to fetch payment');
    }
});

router.post('/', async (req, res) => {
    try {
        const error = validation.validateCreatePayment(req.body);
        if (error) return res.status(400).json({ error });

        const data = await service.createPayment((req as any).userId, req.body);
        res.json(data);
    } catch (err) {
...
router.patch('/:id', async (req, res) => {
    try {
        const data = await service.updatePayment((req as any).userId, req.params.id, req.body);
        res.json(data);
    } catch (err) {
